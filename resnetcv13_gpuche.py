...
import matplotlib.pyplot as plt, pandas as pd, numpy as np, pylab as py, scipy.misc, warnings, os, keras, pydot, time, tensorflow as tf, timeit, matplotlib.pyplot as plt, tarfile, zipfile, shutil, statistics, random, h5py
...
########################################################
from utilities03 import fileremover, filemover, load_data_kfold, get_model, get_callbacks, HighestInteger, ROCCurveCalculate, data_downloader, TestSamplesBalancer, FScoreCalc, remove_weights, last_mover, OverBalancer
########################################################
...
for u in range(0,len(study)):
    try:
        ...
        if TR > 1600:
            y_data, x_data, y_test, x_test, y_val, x_val = OverBalancer(y_data, x_data, vallim, TR, nk)
        else:
    ...
    #############DISTRIBUTION GRAPH#########
        ...
        LisAUC = np.append(LisAUC, [TR, lowauc, mauc, highauc], axis=0)
        print(LisAUC)
        Lism1AUC = np.append(Lism1AUC, [TR, lowauc, mauc, highauc], axis=-1)
        print(Lism1AUC)
...
#######################################################
        ...
        GlobF1s = np.append(GlobF1s, np.percentile(f1s, 50.0))
        GlobF001s = np.append(GlobF001s, np.percentile(f001s, 50.0))
        GloblF1s = np.append(GloblF1s, np.percentile(f1s, 15.87))
        GloblF001s = np.append(GloblF001s, np.percentile(f001s, 15.87))
        GlobhF1s = np.append(GlobhF1s, np.percentile(f1s, 84.13))
        GlobhF001s = np.append(GlobhF001s, np.percentile(f001s, 84.13))

    except AssertionError as error:
        print(error)
    #except:
        #print("\n\n\n ************ WARNING *********** \n\n ** Something went wrong during execution with this amount. Please check it out later. \n\n ** Skipping to next value....")
        #pass
...
print(arr)
arrb = np.array(study, f1s, f001s, Hmauc)
